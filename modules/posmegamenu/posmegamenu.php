...
class Posmegamenu extends Module {
    ...
    public function install() {
        ...
        Configuration::updateValue($this->name . '_top_offset', 110);
    ...
    public function getStaticBlockContent($blockId = NULL, $task = NULL) {
        ...
        if (count($staticBlock) > 0) {
            ...
            $description = str_replace('/pos_wenro/',__PS_BASE_URI__,$description);
            $html .= $description;
        }
        if ($task == 'item') {
    ...
    public function drawCustomMenuBlock($blockId, $bc) {
        ...
        $blockHtml = str_replace('/pos_wenro/',__PS_BASE_URI__,$bc['description']);
        $drawPopup = $blockHtml;
        if ($drawPopup) {
