...
{if isset($import_product) && $import_product}
    <div class="form-group category_default">
        <label class="control-label col-lg-4">{l s='Default category' mod='ets_pres2pres'}</label>
        <div class="col-lg-5">
    ...
    <div class="form-group supplier_default">
        <label class="control-label col-lg-4">{l s='Default Supplier ' mod='ets_pres2pres'}</label>
        <div class="col-lg-5">
            <select name="id_supplier_default">
                <option value="0">{l s='None' mod='ets_pres2pres'}</option>
                {if $suppliers}
    ...
    <div class="form-group manufacturer_default">
        <label class="control-label col-lg-4">{l s='Default manufacturer' mod='ets_pres2pres'}</label>
        <div class="col-lg-5">
...
{if isset($import_cms) && $import_cms}
    <div class="form-group cms_category_default">
        <label class="control-label col-lg-4" for="id_cmscategorydefault">{l s='Default CMS category ' mod='ets_pres2pres'}</label>
        <div class="col-lg-5">
...
<div class="form-group ">
    <label class="control-label col-lg-4"{if $resumeImport} style="display:none;"{/if}>{l s='Delete data before migration' mod='ets_pres2pres'}</label>
    <div class="col-lg-8"{if $resumeImport} style="display:none;"{/if}>
        <span class="switch prestashop-switch fixed-width-lg">
            ...
            <label for="import_delete_before_on">{l s='Yes' mod='ets_pres2pres'}</label>
            ...
            <label for="import_delete_before_off">{l s='No' mod='ets_pres2pres'}</label>
            <a class="slide-button btn"></a>
        </span>
        <p class="help-block">{l s='Enable this option will delete the same kinds of data (that you are migrating) on your website before importing new data so make sure you want to do that' mod='ets_pres2pres'} </p>
    </div>
    <label class="control-label col-lg-4">{l s='Force all ID' mod='ets_pres2pres'}</label>
    <div class="col-lg-8">
        <span class="switch prestashop-switch fixed-width-lg">
            ...
            <label for="import_force_all_id_on">{l s='Yes' mod='ets_pres2pres'}</label>
            ...
            <label for="import_force_all_id_off">{l s='No' mod='ets_pres2pres'}</label>
            <a class="slide-button btn"></a>
        </span>
        <p class="help-block">{l s='Keep ID numbers (such as product ID, customer ID, etc) of source website.' mod='ets_pres2pres'} </p>
    </div>
    {if in_array('employees',$ets_pres2pres_import) || in_array('customers',$ets_pres2pres_import)}
        <label class="control-label col-lg-4">{l s='Keep customer passwords' mod='ets_pres2pres'}</label>
        <div class="col-lg-8">
            <span class="switch prestashop-switch fixed-width-lg">
                ...
                <label for="import_create_new_passwd_off">{l s='Yes' mod='ets_pres2pres'}</label>
                ...
                <label for="import_create_new_passwd_on">{l s='No' mod='ets_pres2pres'}</label>
                <a class="slide-button btn"></a>
            </span>
            <p class="help-block">
                <span class="no_cookie_key">{l s='A password file with all new plain passwords can be downloaded when you complete the import.' mod='ets_pres2pres'}</span>
                <span class="cookie_key">{l s='By enable this option, your customer can use their old password to login on your new shop. You also need to install' mod='ets_pres2pres'} <a href="{$mod_dr_pres2pres|escape:'html':'UTF-8'}plugins/ets_pres2prespwkeeper.zip" target="_blank">{l s='"Prestashop Password Keeper"' mod='ets_pres2pres'}</a>  {l s='module and configure the module with _COOKIE_KEY_ string of the source website. Refer to user guide document for more details.' mod='ets_pres2pres'}</span>
            </p>
        </div>
    {/if}
</div>
