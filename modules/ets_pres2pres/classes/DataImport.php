...
if (!class_exists('Pres2PresReadXML'))
    include_once(_PS_MODULE_DIR_ . 'ets_pres2pres/classes/ReadXMl.php');
...
class Pres2PresDataImport extends Module
{
    ...
    public function importData($file_xml, $class_name, $definition, $foreign_key = array(), $multishop = false)
    {
        ...
        if ($definition['table'] == 'product_attribute' || $definition['table'] == 'tax_rules_group' || $definition['table'] == 'manufacturer' || $definition['table'] == 'supplier'
            || $definition['table'] == 'group' || $definition['table'] == 'feature' || $definition['table'] == 'attribute_group' || $definition['table'] == 'attribute'
            || $definition['table'] == 'image' || $definition['table'] == 'employee' || $definition['table'] == 'cms' || $definition['table'] == 'country'
            || $definition['table'] == 'contact' || $definition['table'] == 'zone') {
                        ...
                        try {
                            ...
                            if ($multishop || (Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE') && Shop::getContext() == Shop::CONTEXT_ALL) || (!$multishop && !Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE'))) {
                                if ($class_name == 'Carrier') {
                                    ...
                                    } elseif ($class_name == 'Combination') {
                                        Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'product_attribute_combination');
                                    } elseif ($class_name == 'CartRule') {
                    ...
                    if (!(int)Db::getInstance()->getValue('SELECT id_new FROM ' . _DB_PREFIX_ . 'ets_pres2pres_' . pSQL($table) . '_import WHERE id_old=' . (int)$data_xml->$primary . ' AND id_import_history="' . (int)$id_import_history . '"', false)) {
                        if ($class_name == 'Employee') {
                            if ($id_employee = (int)Db::getInstance()->getValue('SELECT id_employee FROM ' . _DB_PREFIX_ . 'employee WHERE email="' . pSQL((string)$data_xml->email) . '"', 0)) {
                                ...
                                $import_history->number_import++;
                                Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_pres2pres_import_history SET number_import ="' . (int)$import_history->number_import . '" WHERE id_import_history=' . (int)$id_import_history);
                                continue;
                            }
                        }
                        ...
                        if ($class_name != 'Zone' && $class_name != 'Language' && $class_name != 'Currency' && $class_name != 'ShopGroup' && $class_name != 'Shop' && $class_name != 'ShopUrl' && $class_name != 'Employee' && $class_name != 'Country' && $class_name != 'Carrier') {
                            ...
                            if ($class_name == 'Language') {
                                $object_id = (int)Db::getInstance()->getValue('SELECT id_lang FROM ' . _DB_PREFIX_ . 'lang WHERE iso_code="' . (string)$data_xml->iso_code . '"', 0);
                                if ($object_id) {
                                    Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'ets_pres2pres_' . pSQL($table) . '_import values("","' . (int)$data_xml->$primary . '","' . (int)$object_id . '","' . (int)$id_import_history . '")', false);
                                    $import_history->number_import++;
                                    Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_pres2pres_import_history SET number_import ="' . (int)$import_history->number_import . '" WHERE id_import_history=' . (int)$id_import_history, false);
                                    continue;
                                }
                                ...
                                if (!in_array($language['id_lang'], $list_language_xml)) {
                                    foreach ($fields as $key => $field) {
                                        if (isset($field['lang']) && $field['lang']) {
                                            ...
                                            if (isset($field['validate']) && $field['validate'] && method_exists('Validate', $field['validate']) && isset($temp[$language['id_lang']]) && $temp[$language['id_lang']]) {
                                                $validate = $field['validate'];
                                    ...
                                    if (!$multishop && Shop::getContext() == Shop::CONTEXT_ALL) {
                                        ...
                                        foreach ($shops as $shop) {
                                            ...
                                            if ($object->id_product_attribute == 0 || $this->checkStockAvailable($object->id_product, $object->id_product_attribute, $object->id_shop)) {
                                                ...
                                                $import_history->number_import++;
                                                Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_pres2pres_import_history SET number_import ="' . (int)$import_history->number_import . '" WHERE id_import_history=' . (int)$id_import_history);
                                                Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'ets_pres2pres_' . pSQL($table) . '_import VALUES("","' . (int)$data_xml->$primary . '","' . (int)$data_xml->$primary . '","' . (int)$id_import_history . '")');
                                                continue;
                                            }
                                        }
                                    } else {
                        ...
                        if (isset($object->id) && $object->id) {
                            ...
                            Db::getInstance()->execute('
                                INSERT INTO ' . _DB_PREFIX_ . 'ets_pres2pres_' . pSQL($table) . '_import 
                                VALUES("","' . (int)$data_xml->$primary . '","' . (int)$object->id . '","' . (int)$id_import_history . '")
                            ');
                        } else {
                            $error = 'Insert ' . $class_name . ' with ' . $primary . '=' . (int)$data_xml->$primary . ' error because ' . $primary . ' is null';
                        ...
                        if ($import_history->table_importting != $table) {
                            $import_history->table_importting = $table;
                            Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_pres2pres_import_history SET table_importting ="' . pSQL($import_history->table_importting) . '" WHERE id_import_history=' . (int)$id_import_history);
                            die('imported=' . $table);
                        }
                        if ($import_history->number_import % 100 == 0 || $class_name == 'Image' && $import_history->number_import % 5 == 0 || time() - $start_time > 60) {
                            die('Imported (Auto stop) ' . $class_name . '=' . $import_history->number_import);
                        }
                    }
                }
            }
            ...
            if (isset($xml->$table) && $xml->$table) {
                ...
                foreach ($xml->$table as $data_xml) {
                    ...
                    if (!Db::getInstance()->getRow('SELECT * FROM ' . _DB_PREFIX_ . 'ets_pres2pres_' . pSQL($table) . '_import WHERE id_old=' . (int)$data_xml->$primary . ' AND id_import_history="' . (int)$id_import_history . '"', false)) {
                                ...
                                if ($key != $primary) {
                                    ...
                                    if (isset($fieldsValidate[$key]) && method_exists('Validate', $fieldsValidate[$key]) && $object->$key != '') {
                                        ...
                                        if (!Validate::$validate($object->$key)) {
                                            if ($validate == 'isPrice')
                                                $object->$key = 0;
                                            elseif ($key == 'secure_key' && $class_name == 'Order') {
                                                $object->$key = md5('secure' . rand() . time());
                                            } else
                                                $object->$key = '';
                                        }
                        ...
                        if (isset($object->id) && $object->id) {
                            $imported++;
                            Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_pres2pres_import_history SET number_import ="' . (int)$imported . '",table_importting="' . pSQL($table) . '" WHERE id_import_history=' . (int)$id_import_history);
                            Db::getInstance()->execute('
                                INSERT INTO ' . _DB_PREFIX_ . 'ets_pres2pres_' . pSQL($table) . '_import 
                                VALUES("","' . (int)$data_xml->$primary . '","' . (int)$object->id . '","' . (int)$id_import_history . '")
                            ');
